...
# TODO 将 YOLOv5 的检测部分抽离出来
class Detect_v5:
    def __init__(self):
        pass

    def inference(self, img: np.ndarray):
        # 确保输入的img为np.ndarray类型
        assert isinstance(img, np.ndarray), "img 应是 np.ndarray 类型"
        # 备份原始图片
        img_origin = img.copy()
        # 将图片大小重设为(640, 640)
        or_img = cv2.resize(img, (640, 640))
        # 将BGR格式转为RGB格式并进行通道转换(HWC2CHW)
        ...
        # 使用onnx_session进行模型预测并返回预测结果和原始图片
